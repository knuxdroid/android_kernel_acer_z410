...
#if !defined(ACER_Z410) && !defined(ACER_E800)
...
static ssize_t kpd_store_slid_state(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	switch(slid_enable_status)
    	{
        	case 0 :
			mt_eint_mask(KPD_SLIDE_EINT);
			kpd_print("kpd slid state: KPD_SLIDE_EINT Mask!\n");
     		break;
		case 1 :
			if(slid_eint_reg==0)
			{
				mt_eint_registration(KPD_SLIDE_EINT, KPD_SLIDE_POLARITY,
...
//	kpd_slide_qwerty_init();//API 1 for kpd slide qwerty init settings	
	...
	//mt_eint_set_polarity(KPD_SLIDE_EINT, KPD_SLIDE_POLARITY);
	mt_eint_set_hw_debounce(KPD_SLIDE_EINT, KPD_SLIDE_DEBOUNCE);
	mt_eint_registration(KPD_SLIDE_EINT, KPD_SLIDE_POLARITY, kpd_slide_eint_handler, false);
