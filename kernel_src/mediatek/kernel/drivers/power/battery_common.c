...
extern int charger_compatible_500mA_counter; 
extern int g_is_500mA_charger;
#endif
extern kal_uint32 g_full_check_count;
////////////////////////////////////////////////////////////////////////////////
...
#if defined(ACER_RUNIN_CHARGING)  //for run in test
    ...
    union power_supply_propval *val)
{
    ...
    case POWER_SUPPLY_PROP_ChargerTest:
	    val->intval = g_BAT_ChargerTest;
           printk("[ACER_RUNIN_CHARGING]battery_get_property:val->intval=%d\n",val->intval );
	    break;
#endif
    default:
        ret = -EINVAL;
...
#if defined(ACER_RUNIN_CHARGING)  //for run in test
static int mt_battery_set_property(struct power_supply *psy,
	enum power_supply_property psp,
	union power_supply_propval *val)
{
    int ret = 0;     
	...
	case POWER_SUPPLY_PROP_ChargerTest:
          //if (val->intval == 1)//LK@mask,run in app will close this value when exit 	
          {
              ...
              printk("[ACER_RUNIN_CHARGING][mt_battery_set_property] g_BAT_ChargerTest=%d\n",g_BAT_ChargerTest);
          }
...
static int  mt_battery_property_is_writeable(struct power_supply *psy,enum power_supply_property psp)
{
    int ret = 0;     
...
#if defined(ACER_RUNIN_CHARGING)  //for run in test
    .set_property = mt_battery_set_property,  
    .property_is_writeable = mt_battery_property_is_writeable, 
#endif    
    },
...
static void battery_update(struct battery_data *bat_data)
{
    ...
    if( ((BMT_status.charger_exist == KAL_TRUE) && (BMT_status.bat_charging_state != CHR_ERROR)) ||
        ((BMT_status.charger_exist == KAL_TRUE) && ((BMT_status.bat_temperature_status == BATTERY_UNDER_TEMP) || (BMT_status.bat_temperature_status == BATTERY_OVER_TEMP)))
      )
#else
	if( (BMT_status.charger_exist == KAL_TRUE) && (BMT_status.bat_charging_state != CHR_ERROR) )
#endif
	...
	if(resetBatteryMeter == KAL_TRUE) {
        ...
        #if defined(ACER_RUNIN_CHARGING) //LK@add run in no need to keep UI
            if((g_BAT_ChargerTest == 0) && (bat_is_recharging_phase() == KAL_TRUE))
        #else
            if(bat_is_recharging_phase() == KAL_TRUE)
        #endif
		{
...
{	
	...
	#ifdef BATTERY_NOTIFY_CASE_0006_VBATTEMP_LOW
    if (BMT_status.temperature == ERR_CHARGE_TEMPERATURE)
    {
        battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] ERR_CHARGE_TEMPERATURE NTC fail !!\n\r");         
        BMT_status.bat_charging_state = CHR_ERROR;
        BMT_status.bat_temperature_status=BATTERY_UNDER_TEMP;  
        status = PMU_STATUS_FAIL;       
    }
    else if (BMT_status.temperature < MIN_CHARGE_NOTIFY_TEMPERATURE) //-10
    {
        battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] Battery Under NOTIFY Temperature !!\n\r");          
        BMT_status.bat_charging_state = CHR_ERROR;
        BMT_status.bat_temperature_status = BATTERY_UNDER_NOTIFY_TEMP;  
        status = PMU_STATUS_FAIL;        
    }
	else if (BMT_status.temperature <= MIN_CHARGE_TEMPERATURE)//0 
    {
        battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY]Battery Under Temperature or NTC fail,<= min_temp !!\n\r");  
		BMT_status.bat_temperature_status=BATTERY_UNDER_TEMP;  
		BMT_status.bat_charging_state = CHR_ERROR;
        status = PMU_STATUS_FAIL;       
    }
	else
	{
		...
		{   
            battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] Battery under Temperature recovery ok !!\n\r");                
			BMT_status.bat_charging_state = CHR_PRE;
			BMT_status.bat_temperature_status=0;
		}
	}
    #endif  
    if (BMT_status.temperature >= MAX_CHARGE_NOTIFY_TEMPERATURE) //55
    {
        battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] Battery Under NOTIFY Temperature !!\n\r");          
        BMT_status.bat_charging_state = CHR_ERROR;
        BMT_status.bat_temperature_status = BATTERY_OVER_NOTIFY_TEMP;  
        status = PMU_STATUS_FAIL;        
    }
    else if (BMT_status.temperature >= MAX_CHARGE_TEMPERATURE)//50
    {
        battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] Battery Over Temperature !!\n\r");                
        BMT_status.bat_charging_state = CHR_ERROR;
	...
	else
	{
		...
		{     
           battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] Battery Over Temperature recovery ok !!\n\r");                
			BMT_status.bat_charging_state = CHR_PRE;
			BMT_status.bat_temperature_status=0;
		}
	}
...
static PMU_STATUS mt_battery_CheckChargerVoltage(void)
{
    ...
    if( BMT_status.charger_exist == KAL_TRUE)
    {
		...
		{ 
			...
			{ 
				battery_xlog_printk(BAT_LOG_CRTI, "Power/Battery", "[BATTERY]Charger ovp voltage recovery!!\r\n"); 
...
#if defined(ACER_RUNIN_CHARGING)  //for run in test
static PMU_STATUS mt_battery_CheckRuninState(void)
{
	PMU_STATUS status = PMU_STATUS_OK;
	printk("[ACER_RUNIN_CHARGING] mt_battery_CheckRuninState: g_BAT_ChargerTest=%d\n",g_BAT_ChargerTest);
	printk("[ACER_RUNIN_CHARGING] mt_battery_CheckRuninState: BMT_status.SOC=%d;BMT_status.UI_SOC=%d\n",BMT_status.SOC,BMT_status.UI_SOC);

	if(g_BAT_ChargerTest == 1)
	{
		if (BMT_status.UI_SOC >=50)//LK@SOC->UI_SOC
		{   
			printk("[ACER_RUNIN_CHARGING] BMT_status.UI_SOC >= 50 \n");
			BMT_status.bat_charger_test = BATTERY_TEST_STOP_CHARGING;
			status = PMU_STATUS_FAIL;  
		}
		else if (BMT_status.UI_SOC <= 30)
		{ 
			if (BMT_status.bat_charger_test == BATTERY_TEST_STOP_CHARGING)
			{
				printk("[ACER_RUNIN_CHARGING] BMT_status.UI_SOC <= 30 \n");
	...
	else
	{
		if (BMT_status.bat_charger_test == BATTERY_TEST_STOP_CHARGING)
		{
			printk("[ACER_RUNIN_CHARGING] recharge: bat_charger_test = BATTERY_TEST_STOP_CHARGING\n");
			BMT_status.bat_charger_test = 0;
			BMT_status.bat_charging_state = CHR_PRE;
		}
	}
...
#endif
#if defined(ACER_RUNIN_CHARGING)  //for run in test
	if(mt_battery_CheckRuninState() != PMU_STATUS_OK)
	{
		BMT_status.bat_charging_state = CHR_ERROR;
        return;                  
	}
...
static void mt_battery_notify_VBat_check(void)
{
    ...
    #endif	    
#else
#if defined(ACER_JADEL) //sync with switch charger bq24158 VSAFE
...
static void mt_battery_notify_ICharging_check(void)
{
...
#if defined(ACER_COMMON_DRV)//sync with switch charger bq24158 ISAFE
...
#endif
    {
        ...
        battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] I_charging(%ld) > 1000mA, total time=%d\n", BMT_status.ICharging,BMT_status.total_charging_time);
    }
    else
    {
        g_BatteryNotifyCode &= ~(0x0004);
...
{
#if defined(BATTERY_NOTIFY_CASE_0002_VBATTEMP_HIGH)
...
#if defined(BATTERY_NOTIFY_CASE_0006_VBATTEMP_LOW)
#if defined(ACER_COMMON_DRV)
...
#endif
    {
        ...
        battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] bat_temp(%d) too low\n", BMT_status.temperature);
    }
    else
    {
        g_BatteryNotifyCode &= ~(0x0020);
...
void do_chrdet_int_task(void)
{
    if(g_bat_init_flag == KAL_TRUE)
    {
...
#endif
        {
            ...
            wake_lock(&battery_suspend_lock);
        #if defined(ACER_E800) || defined(ACER_COMMON_DRV)
            printk("[do_chrdet_int_task]:charger exist\n");
            mt6325_upmu_set_isink_ch3_en(0x1); 
        #endif
            #if defined(CONFIG_POWER_EXT)
            mt_usb_connect();
...
#endif
        #if defined(ACER_E800)||defined(ACER_COMMON_DRV)
            //printk("do_chrdet_int_task------");
            mt6325_upmu_set_isink_ch3_en(0x0); 
        #endif
            wake_unlock(&battery_suspend_lock);
...
#if defined(ACER_COMMON_DRV)
	kal_uint32 hv_voltage = 7000*1000;
#else
