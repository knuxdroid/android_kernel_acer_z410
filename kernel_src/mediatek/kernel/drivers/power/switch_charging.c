...
int charger_compatible_500mA_counter = 0; 
int g_is_500mA_charger = 0;
#endif
             ...
             (BMT_status.charger_type == CHARGING_HOST) )
    {
        ...
        //---------------------------------------------------
		 ...
		 if (g_is_500mA_charger == 1)
		 {
			...
			{
//				battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] %s maybe not acer standar charge\r\n",__FUNCTION__);
				g_temp_input_CC_value = CHARGE_CURRENT_500_00_MA;           
...
#define ACER_JADEL_CUST_BATTERY
#endif

#if defined(ACER_JADEL_CUST_BATTERY)
int get_cust_max_chr_cc(void)
{
	int cc;

	if(BMT_status.temperature >= 0 && BMT_status.temperature <= 21){
		cc = CHARGE_CURRENT_675_00_MA; 
	}
	else if (BMT_status.temperature > 21){
		cc = CHARGE_CURRENT_1150_00_MA; 
	}else{
		cc = CHARGE_CURRENT_675_00_MA; 
	}
	return cc;
...
int get_cust_max_chr_cv(void)
{
	int cv;
	if(BMT_status.temperature >= 0 && BMT_status.temperature <= 44){
		cv = BATTERY_VOLT_04_400000_V; 
	}
	else if (BMT_status.temperature > 44){
		cv = BATTERY_VOLT_04_140000_V; 
	}else{
		cv = BATTERY_VOLT_04_140000_V; 
	}
	return cv;
...
//termination current
int get_cust_max_chr_i_term(void)
{
	int it;
	if(BMT_status.temperature >= 0 && BMT_status.temperature <= 44){
		it = 46; 
...
#endif
int set_cust_chr_i_term(int it)
{
//#ifdef MTK_BQ24158_SUPPORT
#if 0  /*use default setting ,120mA*/
	if(it<60){
		bq24158_set_iterm(1);
	}else if(it<121){
		bq24158_set_iterm(2);
...
//modify by willcai 2014-5-29 begin
	...
	#if defined(ACER_JADEL_CUST_BATTERY)
		{
			CHR_CURRENT_ENUM temp;
			temp = get_cust_max_chr_cc();
			if(g_temp_CC_value > temp){
				g_temp_CC_value = temp;
			}
		}
	#endif 
//end
        ...
        {    
			...
			#if defined(ACER_JADEL_CUST_BATTERY)
				{
					BATTERY_VOLTAGE_ENUM temp;
					temp = get_cust_max_chr_cv();
					if(cv_voltage > temp){
						cv_voltage = temp;
					}
				}
	...
	{
		int temp;
		temp = get_cust_max_chr_i_term();
		set_cust_chr_i_term(temp);
	}
	#endif
