...
int read_otp(struct otp_struct *otp_ptr)
{
	...
	if(addr != 0) {
		...
		(*otp_ptr).vcm_id = OV13850_R2A_read_i2c( addr + 2);
		(*otp_ptr).vcm_driver_id = OV13850_R2A_read_i2c( addr + 3);
		(*otp_ptr).otp_mapping_id = OV13850_R2A_read_i2c(addr + 4);
		...
		LOG_INF("Mid=%x lens_id=%x vcm_id=%x vcm_driver_id=%x\n",(*otp_ptr).module_integrator_id,(*otp_ptr).lens_id,(*otp_ptr).vcm_id,(*otp_ptr).vcm_driver_id);
	}
	else {
		...
		(*otp_ptr).lens_id = 0;
		(*otp_ptr).vcm_id = 0;
		(*otp_ptr).vcm_driver_id = 0;
		(*otp_ptr).otp_mapping_id = 0;
	}
	// OTP VCM Calibration
	#if 0   //liuying remove 20141127
	...
	if((otp_flag & 0xc0) == 0x40) {
		...
		LOG_INF(" Lenc calibration group 1 \n");
	}
	else if((otp_flag & 0x30) == 0x10) {
		addr = 0x72ef; // base address of Lenc Calibration group 2
		LOG_INF(" Lenc calibration group 2 \n");
	}
	//
