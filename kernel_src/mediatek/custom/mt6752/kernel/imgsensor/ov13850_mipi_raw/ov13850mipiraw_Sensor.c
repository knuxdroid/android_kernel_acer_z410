...
#define OFILM_MID 0x07           //pei_add
#define XINLI_MID 0x02
#define MCNEX_MID 0x31
...
static imgsensor_info_struct imgsensor_info = {
    ...
    .i2c_addr_table = { 0x20, 0xff},//0x6c  = ov5670  0x6c  //record sensor support all write id addr, only supprt 4must end with 0xff
};
...
EXPORT_SYMBOL(read_cmos_sensor);   //pei_add
...
EXPORT_SYMBOL(write_cmos_sensor);   //pei_add
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
        ...
        do {
            *sensor_id = return_sensor_id();
            if (*sensor_id == imgsensor_info.sensor_id) {
                LOG_INF("pei_debug_get_imgsensor_id2 i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
                return ERROR_NONE;
            }
            LOG_INF("pei_debug_get_imgsensor_id3 Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
            retry--;
        } while(retry > 0);
        i++;
        retry = 2;
	...
	write_cmos_sensor(0x0100, 0x01);////set SCLK for OV13850
	mdelay(1);//
	LOG_INF("pei_add_id1  %s 1 get the module_id from OTP\n",__func__);
	...
	LOG_INF("pei_add_id1  %d , %s 2 get the module_id from OTP\n",module_id,__func__);
	if(module_id == 0x07) 
	{
		LOG_INF("pei_add_id2 is ofilm  module\n");
	}
	else
	{
		LOG_INF("pei_add_id3 is not ofilm module\n");
		*sensor_id = 0xFFFFFFFF;
		return ERROR_SENSOR_CONNECT_FAIL;
	}
...
#if 0
	kal_uint8 module_id = 0x0;
	LOG_INF("pei_add1 get the module_id from OTP\n");
	write_cmos_sensor(0x0100, 0x01);//

	struct otp_struct *otp_ptr1 = (struct otp_struct *)kzalloc(sizeof(struct otp_struct), GFP_KERNEL);
	
	module_id=read_module_info(otp_ptr1);
	LOG_INF("pei_add2 module_id =%d \n", module_id );//pei_add
#endif
	  //for OTP  pei_add 20141222
...
UINT32 OV13850_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)   //pei_add
{
