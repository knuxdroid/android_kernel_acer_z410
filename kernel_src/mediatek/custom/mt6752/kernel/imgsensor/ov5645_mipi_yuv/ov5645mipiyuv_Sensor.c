...
#define LOG_2 LOG_INF("preview 1280*960@30fps,224Mbps/lane; video 1280*960@30,224Mbps/lane; capture 5M@30fps,360Mbps/lane\n")
/****************************   Modify end    *******************************************/
...
static imgsensor_info_struct imgsensor_info = { 
	...
	.pre = {
		...
		.framelength =984, //3168,			//record different mode's framelength
...
static void set_scenemode_portrait()
{
	LOG_INF("E\n");
	write_cmos_sensor(0x3212,0x03);
...
static void set_scenemode_landscape()
{
	LOG_INF("E\n");
	write_cmos_sensor(0x3212,0x03);
	write_cmos_sensor(0x3A00,0x3c); //10-30
...
static void set_scenemode_sunset()
{
	LOG_INF("E\n");
	write_cmos_sensor(0x3212,0x03);
	write_cmos_sensor(0x3A00,0x3c); //10-30
...
static void set_scenemode_sports()
{
	LOG_INF("E\n");
	write_cmos_sensor(0x3212,0x03);
...
static void preview_setting(void)
{
	...
	//;OV5645MIPI 1280x960,30fps
	//56Mhz, 224Mbps/Lane, 2Lane.
	//write_cmos_sensor(0x4202, 0x0f);//	; stop mipi stream
	write_cmos_sensor(0x300e, 0x45);//	; MIPI 2 lane
	write_cmos_sensor(0x3034, 0x18);// PLL, MIPI 8-bit mode
	write_cmos_sensor(0x3035, 0x21);// PLL
	write_cmos_sensor(0x3036, 0x70);// PLL
	...
	write_cmos_sensor(0x4514, 0x00);
	if(imgsensor.scene_mode==SCENE_MODE_NIGHTSCENE)
	{
		write_cmos_sensor(0x3a02, 0x17);//	; max exp 60 = 740
		write_cmos_sensor(0x3a03, 0x10);//	; max exp 60
		write_cmos_sensor(0x3a14, 0x17);//	; max exp 50 = 740
		write_cmos_sensor(0x3a15, 0x10);//	; max exp 50
	}
	else
	{
		write_cmos_sensor(0x3a02, 0x0b);//	; max exp 60 = 740
		write_cmos_sensor(0x3a03, 0x88);//	; max exp 60
		write_cmos_sensor(0x3a14, 0x0b);//	; max exp 50 = 740
		write_cmos_sensor(0x3a15, 0x88);//	; max exp 50
	}
	write_cmos_sensor(0x3a08, 0x01);//	; B50 = 222
	write_cmos_sensor(0x3a09, 0x27);//	; B50
	write_cmos_sensor(0x3a0a, 0x00);//	; B60 = 185
	...
	write_cmos_sensor(0x3032, 0x00);//
	write_cmos_sensor(0x4000, 0x89);//
	write_cmos_sensor(0x3a00, 0x3c);//	; ae mode	
	write_cmos_sensor(0x5302, 0x20);//	; sharpen MT
	write_cmos_sensor(0x5303, 0x18);//	; sharpen MT 
	write_cmos_sensor(0x5306, 0x08);//	; DNS off1  
	write_cmos_sensor(0x5307, 0x10);//	; DNS off2  
}

static void set_scenemode_night()
{
	LOG_INF("E\n");
	write_cmos_sensor(0x3212,0x03);
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x3103, 0x11);//	; PLL clock selection
	write_cmos_sensor(0x3008, 0x82);//	; software reset	
	mDELAY(5);            										//; delay 2ms                           					
	write_cmos_sensor(0x3008, 0x42);//	; software power down
	write_cmos_sensor(0x3103, 0x03);//	; clock from PLL
	write_cmos_sensor(0x3503, 0x07);//	; AGC manual, AEC manual
	write_cmos_sensor(0x3406, 0x01);//	; awb manual, AEC manual
    write_cmos_sensor(0x3000, 0x30);
    write_cmos_sensor(0x3004, 0xef);
	write_cmos_sensor(0x3002, 0x1c);//	; system reset
	write_cmos_sensor(0x3006, 0xc3);//	; clock enable
	write_cmos_sensor(0x300e, 0x45);//	; MIPI 2 lane
	write_cmos_sensor(0x3017, 0x40);//	; Frex, CSK input, Vsync output
	write_cmos_sensor(0x3018, 0x00);//	; GPIO input
	write_cmos_sensor(0x302c, 0x02);//	; GPIO input
	...
	write_cmos_sensor(0x3618, 0x00);//
	write_cmos_sensor(0x4800, 0x24);//chage mipi data free/gate
	write_cmos_sensor(0x3034, 0x18);//	; PLL, MIPI 8-bit mode
	write_cmos_sensor(0x3035, 0x21);//	; PLL
	write_cmos_sensor(0x3036, 0x70);//	; PLL
	write_cmos_sensor(0x3037, 0x13); // ; PLL
	...
	write_cmos_sensor(0x3824, 0x01); // ; PLL
	write_cmos_sensor(0x460c, 0x20); // ; PLL
	write_cmos_sensor(0x3500, 0x00);//	; exposure = 0x100
	write_cmos_sensor(0x3501, 0x01);//	; exposure
	write_cmos_sensor(0x3502, 0x00);//	; exposure
	write_cmos_sensor(0x350a, 0x00);//	; gain = 0x3f
	write_cmos_sensor(0x350b, 0x3f);//	; gain
	
	write_cmos_sensor(0x3400, 0x06); // ; PLL
	write_cmos_sensor(0x3401, 0x02);//	; exposure = 0x100
	write_cmos_sensor(0x3402, 0x04);//	; exposure
	write_cmos_sensor(0x3403, 0x00);//	; exposure
	write_cmos_sensor(0x3404, 0x05);//	; gain = 0x3f
	write_cmos_sensor(0x3405, 0x00);//	; gain
	...
	write_cmos_sensor(0x3804, 0x0a);//	; HW = 2623
	write_cmos_sensor(0x3805, 0x3f);//	; HW
	write_cmos_sensor(0x3806, 0x07);//	; VH = 1705
	write_cmos_sensor(0x3807, 0x9d);//	; VH
	write_cmos_sensor(0x3808, 0x05);//	; DVPHO = 1280
	write_cmos_sensor(0x3809, 0x00);//	; DVPHO
	write_cmos_sensor(0x380a, 0x03);//	; DVPHO
	write_cmos_sensor(0x380b, 0xc0);//	; DVPVO
	write_cmos_sensor(0x380c, 0x07);//	; HTS = 2160
	write_cmos_sensor(0x380d, 0x68);//	; HTS
	write_cmos_sensor(0x380e, 0x03);//	; VTS = 740
	...
	write_cmos_sensor(0x3a14, 0x03); // ; max exp 50 = 740
	write_cmos_sensor(0x3a15, 0xd8);//	; max exp 50
	write_cmos_sensor(0x3a18, 0x00);//	; gain ceiling = 15.5x
	write_cmos_sensor(0x3a19, 0x60);//	; gain ceiling
	write_cmos_sensor(0x3a05, 0x30);//	; enable band insert, ken,  
	write_cmos_sensor(0x3c01, 0xb4); // ;manual banding mode
	write_cmos_sensor(0x3c00, 0x04); // ;50 Banding mode 
	...
	write_cmos_sensor(0x4004, 0x02);//	; BLC line number
	write_cmos_sensor(0x4005, 0x18);//	; BLC update triggered by gain change
	write_cmos_sensor(0x4050, 0x6e);//	; BLC line number
	write_cmos_sensor(0x4051, 0x8f);//	; BLC update triggered by gain change
	...
	write_cmos_sensor(0x5001, 0xa3);//	; SDE on, scale off, UV adjust off, color matrix on, AWB on
	write_cmos_sensor(0x5002, 0x80);//   
	write_cmos_sensor(0x501d, 0x00);//
	...
	write_cmos_sensor(0x3008, 0x02);//	; wake up from software standby
	write_cmos_sensor(0x3503, 0x00);//	; AGC on, AEC on
	write_cmos_sensor(0x5180, 0xff);// ;awb
	write_cmos_sensor(0x5181, 0xf3);//
	write_cmos_sensor(0x5182, 0x0 );//
	write_cmos_sensor(0x5183, 0x14);//
	write_cmos_sensor(0x5184, 0x25);//
	write_cmos_sensor(0x5185, 0x24);//
	write_cmos_sensor(0x5186, 0xe );//
	write_cmos_sensor(0x5187, 0x10);//
	write_cmos_sensor(0x5188, 0xb );//
	write_cmos_sensor(0x5189, 0x74);//
	write_cmos_sensor(0x518a, 0x54);//
	write_cmos_sensor(0x518b, 0xeb);//
	write_cmos_sensor(0x518c, 0xa8);//
	write_cmos_sensor(0x518d, 0x36);//
	write_cmos_sensor(0x518e, 0x2d);//
	write_cmos_sensor(0x518f, 0x51);//
	write_cmos_sensor(0x5190, 0x40);//
	write_cmos_sensor(0x5191, 0xf8);//
	write_cmos_sensor(0x5192, 0x4 );//
	write_cmos_sensor(0x5193, 0x70);//
	write_cmos_sensor(0x5194, 0xf0);//
	write_cmos_sensor(0x5195, 0xf0);//
	write_cmos_sensor(0x5196, 0x3 );//
	write_cmos_sensor(0x5197, 0x1 );//
	write_cmos_sensor(0x5198, 0x5 );//
	write_cmos_sensor(0x5199, 0xe5);//
	write_cmos_sensor(0x519a, 0x4 );//
	write_cmos_sensor(0x519b, 0x0 );//
	write_cmos_sensor(0x519c, 0x4 );//
	write_cmos_sensor(0x519d, 0x8f);//
	...
	write_cmos_sensor(0x538a, 0x01);//
	write_cmos_sensor(0x538b, 0x98);//
	write_cmos_sensor(0x5300, 0x08);//	; sharpen MT th1
	write_cmos_sensor(0x5301, 0x30);//	; sharpen MT th2
	write_cmos_sensor(0x5302, 0x18);//	; sharpen MT off1
	write_cmos_sensor(0x5303, 0x08);//	; sharpen MT off2
	write_cmos_sensor(0x5304, 0x08);//	; DNS th1
	write_cmos_sensor(0x5305, 0x30);//	; DNS th2
	write_cmos_sensor(0x5306, 0x08);//	; DNS off1
	write_cmos_sensor(0x5307, 0x16);//	; DNS off2
	write_cmos_sensor(0x5309, 0x08);//	; sharpen TH th1
	write_cmos_sensor(0x530a, 0x30);//	; sharpen TH th2
	write_cmos_sensor(0x530b, 0x04);//	; sharpen TH th2
	write_cmos_sensor(0x530c, 0x06);//	; sharpen TH off2
	write_cmos_sensor(0x5480, 0x01);//	; bias on
	write_cmos_sensor(0x5481, 0x08);//	; Y yst 00
	write_cmos_sensor(0x5482, 0x14);//
	write_cmos_sensor(0x5483, 0x28);//
	write_cmos_sensor(0x5484, 0x51);//
	...
	write_cmos_sensor(0x5583, 0x40);//
	write_cmos_sensor(0x5584, 0x28);//
	write_cmos_sensor(0x5589, 0x18);//
	write_cmos_sensor(0x558a, 0x00);//
	write_cmos_sensor(0x558b, 0x3c);//
	write_cmos_sensor(0x5800, 0x3F);//lsc
	write_cmos_sensor(0x5801, 0x1D);//
	write_cmos_sensor(0x5802, 0x19);//
	write_cmos_sensor(0x5803, 0x18);//
	write_cmos_sensor(0x5804, 0x1E);//
	write_cmos_sensor(0x5805, 0x3F);//
	write_cmos_sensor(0x5806, 0x12);//
	write_cmos_sensor(0x5807, 0x0D);//
	write_cmos_sensor(0x5808, 0x09);//
	write_cmos_sensor(0x5809, 0x09);//
	write_cmos_sensor(0x580a, 0x0D);//
	write_cmos_sensor(0x580b, 0x11);//
	write_cmos_sensor(0x580c, 0x0E);//
	write_cmos_sensor(0x580d, 0x04);//
	write_cmos_sensor(0x580e, 0x01);//
	write_cmos_sensor(0x580f, 0x01);//
	write_cmos_sensor(0x5810, 0x05);//
	write_cmos_sensor(0x5811, 0x0D);//
	write_cmos_sensor(0x5812, 0x0B);//
	write_cmos_sensor(0x5813, 0x04);//
	write_cmos_sensor(0x5814, 0x00);//
	write_cmos_sensor(0x5815, 0x00);//
	write_cmos_sensor(0x5816, 0x04);//
	write_cmos_sensor(0x5817, 0x0B);//
	write_cmos_sensor(0x5818, 0x12);//
	write_cmos_sensor(0x5819, 0x0A);//
	write_cmos_sensor(0x581a, 0x06);//
	write_cmos_sensor(0x581b, 0x06);//
	write_cmos_sensor(0x581c, 0x0A);//
	write_cmos_sensor(0x581d, 0x10);//
	write_cmos_sensor(0x581e, 0x2F);//
	write_cmos_sensor(0x581f, 0x17);//
	write_cmos_sensor(0x5820, 0x13);//
	write_cmos_sensor(0x5821, 0x13);//
	write_cmos_sensor(0x5822, 0x18);//
	write_cmos_sensor(0x5823, 0x33);//
	write_cmos_sensor(0x5824, 0x84);//
	write_cmos_sensor(0x5825, 0x27);//
	write_cmos_sensor(0x5826, 0x29);//
	write_cmos_sensor(0x5827, 0x27);//
	write_cmos_sensor(0x5828, 0x56);//
	write_cmos_sensor(0x5829, 0x47);//
	write_cmos_sensor(0x582a, 0x45);//
	write_cmos_sensor(0x582b, 0x54);//
	write_cmos_sensor(0x582c, 0x55);//
	write_cmos_sensor(0x582d, 0x27);//
	...
	write_cmos_sensor(0x5833, 0x37);//
	write_cmos_sensor(0x5834, 0x55);//
	write_cmos_sensor(0x5835, 0x63);//
	write_cmos_sensor(0x5836, 0x45);//
	write_cmos_sensor(0x5837, 0x17);//
	write_cmos_sensor(0x5838, 0x56);//
	write_cmos_sensor(0x5839, 0x2A);//
	write_cmos_sensor(0x583a, 0x0A);//
	write_cmos_sensor(0x583b, 0x18);//
	write_cmos_sensor(0x583c, 0x36);//
	write_cmos_sensor(0x583d, 0xAE);//
	write_cmos_sensor(0x583e, 0x10);//
	write_cmos_sensor(0x583f, 0x08);//
	write_cmos_sensor(0x5840, 0x00);//
	write_cmos_sensor(0x5025, 0x00);//
	write_cmos_sensor(0x3a00, 0x3c);//	; ae mode	
	write_cmos_sensor(0x3a0f, 0x30);//	; AEC in H
	write_cmos_sensor(0x3a10, 0x28);//	; AEC in L
	write_cmos_sensor(0x3a1b, 0x30);//	; AEC out H
	write_cmos_sensor(0x3a1e, 0x28);//	; AEC out L
	write_cmos_sensor(0x3a11, 0x60);//	; control zone H
	write_cmos_sensor(0x3a1f, 0x14);//	; control zone L
}
static void pip_capture_setting()
{
	...
	LOG_INF( "Enter!\n");
	write_cmos_sensor(0x4202, 0x0f);//	; stop mipi stream
	write_cmos_sensor(0x3a00, 0x38);//	; max exp 60 = 740
	write_cmos_sensor(0x3a02, 0x03);//	; max exp 60 = 740
	write_cmos_sensor(0x3a03, 0xd8);//	; max exp 60
	write_cmos_sensor(0x3a14, 0x03);//	; max exp 50 = 740
	write_cmos_sensor(0x3a15, 0xd8);//	; max exp 50
	...
	write_cmos_sensor(0x300e, 0x45);//	; MIPI 2 lane
	write_cmos_sensor(0x3034, 0x18); //PLL, MIPI 8-bit mode
	write_cmos_sensor(0x3035, 0x11); //PLL
	write_cmos_sensor(0x3036, 0x5a); //PLL
	write_cmos_sensor(0x3037, 0x13); //PLL
	write_cmos_sensor(0x3108, 0x01); //PLL
	write_cmos_sensor(0x3824, 0x01); //PLL
	write_cmos_sensor(0x460c, 0x20); //PLL
	...
	write_cmos_sensor(0x3800, 0x00); //HS = 0
	write_cmos_sensor(0x3801, 0x00); //HS
	write_cmos_sensor(0x3802, 0x00); //VS = 0
	write_cmos_sensor(0x3803, 0x00); //VS
	write_cmos_sensor(0x3804, 0x0a); //HW = 2623
	write_cmos_sensor(0x3805, 0x3f);//	; HW
	write_cmos_sensor(0x3806, 0x07);//	; VH = 1705
	write_cmos_sensor(0x3807, 0x9f);//	; VH
	write_cmos_sensor(0x3808, 0x0a);//	; DVPHO = 2560
	write_cmos_sensor(0x3809, 0x20);//	; DVPHO
	write_cmos_sensor(0x380a, 0x07);//	; DVPVO = 1440
	write_cmos_sensor(0x380b, 0x98);//	; DVPVO
	write_cmos_sensor(0x380c, 0x0b);//	; HTS = 2984
	write_cmos_sensor(0x380d, 0xec);//	; HTS
	write_cmos_sensor(0x380e, 0x07);//	; VTS = 1464
	write_cmos_sensor(0x380f, 0xb0);//	; VTS
	write_cmos_sensor(0x3810, 0x00); //H OFF = 16
	write_cmos_sensor(0x3811, 0x10); //H OFF
	write_cmos_sensor(0x3812, 0x00); //V OFF = 4
	write_cmos_sensor(0x3813, 0x06);//	; V OFF
	...
	write_cmos_sensor(0x3821, 0x06);//	; mirror on, H bin off
	write_cmos_sensor(0x4514, 0x00);
	write_cmos_sensor(0x3a0e, 0x06);//	; flip off, V bin off
	write_cmos_sensor(0x3a0d, 0x08);//	; mirror on, H bin off
	write_cmos_sensor(0x4004, 0x06);//	; BLC line number
	write_cmos_sensor(0x4837, 0x16);//; MIPI global timing  
	write_cmos_sensor(0x503d, 0x00);//
	write_cmos_sensor(0x5000, 0xa7);//
	...
	write_cmos_sensor(0x350c, 0x00);
	write_cmos_sensor(0x350d, 0x00);
	write_cmos_sensor(0x5302, 0x28);//	; sharpen MT
	write_cmos_sensor(0x5303, 0x20);//	; sharpen MT 
	write_cmos_sensor(0x5306, 0x08);//	; DNS off1	
	write_cmos_sensor(0x5307, 0x10);//	; DNS off2 
	write_cmos_sensor(0x4202, 0x00);//	; stop mipi stream
}
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	set_mirror_flip(IMAGE_V_MIRROR);//IMAGE_NORMAL
        #else
...
static void awb_calibattion(kal_uint32 color_r_gain,kal_uint32 color_b_gain)
{
	...
	if(color_r_gain>0x600)//d65   66b 5a7 4a7 449	//\B0칫\CA\D2
	{
		 if (interval>=0x70)
		 {
	...
	else if(color_r_gain>0x540)//cwf  //\B4\B0\BB\A7\CD\E2
	{
		if (interval>=0x70)
		{
			color_r_gain_w=color_r_gain *97/100;																			   
...
static void get_evawb_ref(UINT32 pSensorAEAWBRefStruct)
{
	LOG_INF("E \n");
...
static void get_curaeawb_Info(UINT32 pSensorAEAWBCurStruct)
{
	LOG_INF("E \n");
	PSENSOR_AE_AWB_CUR_STRUCT Info = (PSENSOR_AE_AWB_CUR_STRUCT)pSensorAEAWBCurStruct;
	Info->SensorAECur.AeCurShutter=get_shutter();
	Info->SensorAECur.AeCurGain=get_gain() ;
	Info->SensorAwbGainCur.AwbCurRgain=((read_cmos_sensor(0x3401)&&0xff)+((read_cmos_sensor(0x3400)&&0xff)*256));
	Info->SensorAwbGainCur.AwbCurBgain=((read_cmos_sensor(0x3405)&&0xff)+((read_cmos_sensor(0x3404)&&0xff)*256));
}
...
static void get_aeawb_lock_info(UINT32 *pAElockRet32, UINT32 *pAWBlockRet32)
{
	*pAElockRet32 = 1;
	*pAWBlockRet32 = 1;
	LOG_INF("pAElockRet32 = %s,pAWBlockRet32=%s\n", *pAElockRet32,*pAWBlockRet32);
}
static void get_delay_Info(UINT32 delayAddr)
{
	LOG_INF("E \n");
...
{ 	  
    *pFeatureReturnPara32 = 1;    //0 is means it is not support touch af,1 is means it is support touch af 	
}

static void get_ae_max_num_metering_areas(UINT32 *pFeatureReturnPara32)
{
	*pFeatureReturnPara32 = 1;    //0 is means it is not support touch ae,1 is means it is support touch ae 	
}
...
static set_ae_block(unsigned int x0,unsigned int y0)
{	
	unsigned int i,j;
	LOG_INF("E\n x0=%d,y0=%d",x0,y0);
	for(i=0;i<AE_WIN_MUN_W;i++)
...
{	
    ...
    temp=0x11;
    if(ae_pline_table[0][0]==TRUE)    { temp=temp|0x0F;}
    if(ae_pline_table[0][1]==TRUE)    { temp=temp|0xF0;}
    //write 0x5688
    write_cmos_sensor(0x5688,temp);
    temp=0x11;
    if(ae_pline_table[0][2]==TRUE)    { temp=temp|0x0F;}
    if(ae_pline_table[0][3]==TRUE)    { temp=temp|0xF0;}
    //write 0x5689
    write_cmos_sensor(0x5689,temp);
    temp=0x11;
    if(ae_pline_table[1][0]==TRUE)    { temp=temp|0x0F;}
    if(ae_pline_table[1][1]==TRUE)    { temp=temp|0xF0;}
    //write 0x568A
    write_cmos_sensor(0x568A,temp);
    temp=0x11;
    if(ae_pline_table[1][2]==TRUE)    { temp=temp|0x0F;}
    if(ae_pline_table[1][3]==TRUE)    { temp=temp|0xF0;}
    //write 0x568B
    write_cmos_sensor(0x568B,temp);
    temp=0x11;
    if(ae_pline_table[2][0]==TRUE)    { temp=temp|0x0F;}
    if(ae_pline_table[2][1]==TRUE)  { temp=temp|0xF0;}
	//write 0x568C
    write_cmos_sensor(0x568C,temp);
    temp=0x11;
    if(ae_pline_table[2][2]==TRUE)    { temp=temp|0x0F;}
    if(ae_pline_table[2][3]==TRUE)    { temp=temp|0xF0;}
    //write 0x568D
    write_cmos_sensor(0x568D,temp);    
    temp=0x11;
    if(ae_pline_table[3][0]==TRUE)    { temp=temp|0x0F;}
    if(ae_pline_table[3][1]==TRUE)    { temp=temp|0xF0;}
	//write 0x568E
    write_cmos_sensor(0x568E,temp);
    temp=0x11;
    if(ae_pline_table[3][2]==TRUE)    { temp=temp|0x0F;}
    if(ae_pline_table[3][3]==TRUE)    { temp=temp|0xF0;}
	//write 0x568F
...
static void print_ae_planetable(void)
{
    ...
    for(j=0;j<AE_WIN_MUN_H;j++)
	{
		for(i=0;i<AE_WIN_MUN_W;i++)
		{
			LOG_INF("ae_pline_table=%d,i=%d,j=%d",ae_pline_table[j][i],i,j);
		}
	}
}
	...
	switch (feature_id)
	{
		...
		*feature_para_len=4;
		...
		case SENSOR_FEATURE_SET_NIGHTMODE:
			set_scenemode_night((BOOL) *feature_data_16);
			break;
		case SENSOR_FEATURE_SET_MIN_MAX_FPS:
			set_max_min_fps((UINT32)*feature_data_32, (UINT32)*(feature_data_32+1));
			break;
		/**********************Strobe Ctrl Start *******************************/
		...
		case SENSOR_FEATURE_GET_AE_FLASHLIGHT_INFO:
			get_flash_Info(*feature_data_32);
			break;
		...
		case SENSOR_FEATURE_SET_YUV_CMD:
			set_yuv_cmd((FEATURE_ID)*feature_data_32, *(feature_data_32+1));
			break;	
		case SENSOR_FEATURE_SET_YUV_3A_CMD:
            set_3actrl_info((ACDK_SENSOR_3A_LOCK_ENUM)*feature_data_32);
            break;
		...
		case SENSOR_FEATURE_GET_EV_AWB_REF:
			get_evawb_ref(*feature_data_32);
			break;		
		case SENSOR_FEATURE_GET_SHUTTER_GAIN_AWB_GAIN:
			get_curaeawb_Info(*feature_data_32);			
			break;
		case SENSOR_FEATURE_GET_EXIF_INFO:
       		get_exifInfo(*feature_data_32);
            break;
		case SENSOR_FEATURE_GET_DELAY_INFO:
			get_delay_Info(*feature_data_32);
			break;
        ...
        case SENSOR_FEATURE_MOVE_FOCUS_LENS:
             //af_move_to(feature_data_32); // not implement yet.
             break;
        ...
        case SENSOR_FEATURE_SET_AF_WINDOW:     
             set_af_window(*feature_data_32,imgsensor_info.pre.grabwindow_width, imgsensor_info.pre.grabwindow_height);
             break;
        case SENSOR_FEATURE_SET_AE_WINDOW:
             set_ae_window(*feature_data_32,imgsensor_info.pre.grabwindow_width, imgsensor_info.pre.grabwindow_height);
             break;
             break;
        case SENSOR_FEATURE_GET_AE_AWB_LOCK_INFO:
             get_aeawb_lock_info((*feature_data_32),*(feature_data_32+1));
             break;
		default:
			break;			
	}
