...
#include "s5k3l2xx_setting.h"

static DEFINE_SPINLOCK(s5k3l2xxmipiraw_drv_lock);

#define S5K3L2XX_DEBUG
#define S5K3L2XX_TEST_PATTERN_CHECKSUM (0xc3dd012b)
...
kal_uint32 S5K3L2XX_FRAMELENGTH_LOCK = s5k3l2xx_video_frame_length;
kal_uint32 S5K3L2XX_DUMMY_LOCK = 0;
...
{{ 4208, 3120, 0000, 0000, 4208, 3120, 2104, 1560, 0000, 0000, 2104, 1560, 0003, 0003, 2096, 1552}, // Preview 
 ...
 { 4208, 3120, 0000, 0000, 4208, 3120, 2104, 1560, 0000, 0000, 2104, 1560, 0003, 0003, 2096, 1552}, // Custom1 (defaultuse preview) 
 { 4208, 3120, 0000, 0000, 4208, 3120, 2104, 1560, 0000, 0000, 2104, 1560, 0003, 0003, 2096, 1552}, // Custom2 
 { 4208, 3120, 0000, 0000, 4208, 3120, 2104, 1560, 0000, 0000, 2104, 1560, 0003, 0003, 2096, 1552}, // Custom3 
 { 4208, 3120, 0000, 0000, 4208, 3120, 2104, 1560, 0000, 0000, 2104, 1560, 0003, 0003, 2096, 1552}, // Custom4 
 { 4208, 3120, 0000, 0000, 4208, 3120, 2104, 1560, 0000, 0000, 2104, 1560, 0003, 0003, 2096, 1552}, // Custom5 
 };// slim video  
...
kal_uint16 S5K3L2XX_get_framelength(void)
{
	...
	if(s5k3l2xx.sensorMode == SENSOR_MODE_PREVIEW)
		...
		Framelength = s5k3l2xx_video_frame_length;
	else if(s5k3l2xx.sensorMode == IMGSENSOR_MODE_HIGH_SPEED_VIDEO)
		Framelength = s5k3l2xx_hs_video_frame_length;
	else if(s5k3l2xx.sensorMode == IMGSENSOR_MODE_SLIM_VIDEO)
		Framelength = s5k3l2xx_slim_video_frame_length;
	return Framelength;
}
...
kal_uint16 S5K3L2XX_get_linelength(void)
{
	...
	if(s5k3l2xx.sensorMode == SENSOR_MODE_PREVIEW)
		...
		Linelength = s5k3l2xx_hs_video_line_length;
	else if(s5k3l2xx.sensorMode == IMGSENSOR_MODE_SLIM_VIDEO)
		Linelength = s5k3l2xx_slim_video_line_length;
	return Linelength;
}
...
kal_uint32 S5K3L2XX_get_pixelclock(void)
{
	...
	if(s5k3l2xx.sensorMode == SENSOR_MODE_PREVIEW)
		...
		PixelClock = s5k3l2xx_hs_video_pixelclock;
	else if(s5k3l2xx.sensorMode == IMGSENSOR_MODE_SLIM_VIDEO)
		PixelClock = s5k3l2xx_slim_video_pixelclock;
	return PixelClock;
}
...
void S5K3L2XX_write_shutter(kal_uint32 shutter)
{
	...
	if(s5k3l2xx.sensorMode == IMGSENSOR_MODE_HIGH_SPEED_VIDEO)
		frame_length_min = S5K3L2XX_get_framelength();
	else
		frame_length_min = S5K3L2XX_FRAMELENGTH_LOCK;
	S5K3L2XXDB("frame_length_min=%d, sensorMode=%d\n", frame_length_min, s5k3l2xx.sensorMode);
	if(frame_length < frame_length_min)
		frame_length = frame_length_min;
	
	if((s5k3l2xx.S5K3L2XXAutoFlickerMode == KAL_TRUE)&&(s5k3l2xx.sensorMode != IMGSENSOR_MODE_HIGH_SPEED_VIDEO))
	{
		line_length = S5K3L2XX_get_linelength();
		pixelclock = S5K3L2XX_get_pixelclock();
		framerate = pixelclock / (frame_length * line_length) * 10;
		  
		if(framerate > 290)
		{
		  	framerate = 290;
		  	frame_length = pixelclock / (framerate * line_length) * 10;
		}
...
static void check_output_stream_on(void)
{
	...
	{		
		read_register0005_value = S5K3L2XX_read_cmos_sensor(0x0005);		
		S5K3L2XXDB("read_count= %d, read_register0005_value = 0x%x\n", read_count, read_register0005_value);		
...
static void check_output_stream_off(void)
{
	...
	{		
		...
		if(read_count == 4)			
			S5K3L2XXDB("[ERROR]stream off\n");	
	}
}

static void write_setting(kal_uint32 setting_table[][3])
{
	kal_uint32 i = 0, regaddr, regdata, bitorder;
	
	while(setting_table[i][2]!=0x0003)
	{
		regaddr = setting_table[i][0];
		regdata = setting_table[i][1];
		bitorder= setting_table[i][2];

		if(bitorder== 0x0002)
			Sleep(regdata);
		else if(bitorder == 0x0001)
			S5K3L2XX_write_cmos_sensor1(regaddr, regdata);
		else if(bitorder == 0x0000)
			S5K3L2XX_write_cmos_sensor(regaddr, regdata);
		else if(bitorder == 0x0004)
			check_output_stream_off();
		else if(bitorder == 0x0005)
...
{
	write_setting(init_setting_register_table);
	return;
#if 0
...
{
	write_setting(preview_setting_register_table);
	return;
#if 0
   	kal_uint16 read_count = 0, read_register0005_value = 0;
...
{
	write_setting(capture_setting_register_table);
	return;
#if 0
	kal_uint16 read_count = 0, read_register0005_value = 0;
...
UINT32 S5K3L2XXPreview(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window, MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	S5K3L2XX_FRAMELENGTH_LOCK = s5k3l2xx_preview_frame_length;
	S5K3L2XX_DUMMY_LOCK = 1;
	spin_unlock(&s5k3l2xxmipiraw_drv_lock);
...
UINT32 S5K3L2XXVideo(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window, MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	s5k3l2xx.imgMirror = sensor_config_data->SensorImageMirror;
	S5K3L2XX_FRAMELENGTH_LOCK = s5k3l2xx_video_frame_length;
	S5K3L2XX_DUMMY_LOCK = 1;
	spin_unlock(&s5k3l2xxmipiraw_drv_lock);
...
UINT32 S5K3L2XXCapture(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window, MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	else
	{
		...
		S5K3L2XX_FRAMELENGTH_LOCK = s5k3l2xx_capture_frame_length;
		S5K3L2XX_DUMMY_LOCK = 1;
		spin_unlock(&s5k3l2xxmipiraw_drv_lock);
...
UINT32 S5K3L2XXGetResolution(MSDK_SENSOR_RESOLUTION_INFO_STRUCT *pSensorResolution)
{
    ...
    pSensorResolution->SensorHighSpeedVideoWidth     = s5k3l2xx_hs_video_width;
    pSensorResolution->SensorHighSpeedVideoHeight    = s5k3l2xx_hs_video_height;
    
    pSensorResolution->SensorSlimVideoWidth  = s5k3l2xx_slim_video_width;
    pSensorResolution->SensorSlimVideoHeight     = s5k3l2xx_slim_video_height;
...
UINT32 S5K3L2XXGetInfo(MSDK_SCENARIO_ID_ENUM ScenarioId, MSDK_SENSOR_INFO_STRUCT *pSensorInfo, MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
	...
	pSensorInfo->HighSpeedVideoDelayFrame = 3;
    pSensorInfo->SlimVideoDelayFrame = 3;
    ...
    switch (ScenarioId)
    {
		...
		case MSDK_SCENARIO_ID_HIGH_SPEED_VIDEO:
            pSensorInfo->SensorClockFreq=s5k3l2xx_master_clock;
            pSensorInfo->SensorClockRisingCount= 0;
			pSensorInfo->SensorGrabStartX = s5k3l2xx_hs_video_startx;	
            pSensorInfo->SensorGrabStartY = s5k3l2xx_hs_video_starty;	
            pSensorInfo->SensorMIPILaneNumber = SENSOR_MIPI_4_LANE;
            pSensorInfo->MIPIDataLowPwr2HighSpeedTermDelayCount = 0;
            pSensorInfo->MIPIDataLowPwr2HighSpeedSettleDelayCount = 20;
            pSensorInfo->MIPICLKLowPwr2HighSpeedTermDelayCount = 0;
        ...
        case MSDK_SCENARIO_ID_SLIM_VIDEO:
			pSensorInfo->SensorClockFreq=s5k3l2xx_master_clock;
            pSensorInfo->SensorClockRisingCount= 0;
			pSensorInfo->SensorGrabStartX = s5k3l2xx_slim_video_startx;	
            pSensorInfo->SensorGrabStartY = s5k3l2xx_slim_video_starty;	
            pSensorInfo->SensorMIPILaneNumber = SENSOR_MIPI_4_LANE;
            pSensorInfo->MIPIDataLowPwr2HighSpeedTermDelayCount = 0;
            pSensorInfo->MIPIDataLowPwr2HighSpeedSettleDelayCount = 20;
            pSensorInfo->MIPICLKLowPwr2HighSpeedTermDelayCount = 0;
...
{
	if(s5k3l2xx.sensorMode !=  IMGSENSOR_MODE_HIGH_SPEED_VIDEO)
		write_setting(vga_120fps_setting_register_table);
	
	spin_lock(&s5k3l2xxmipiraw_drv_lock);
	s5k3l2xx.sensorMode = IMGSENSOR_MODE_HIGH_SPEED_VIDEO; 
	spin_unlock(&s5k3l2xxmipiraw_drv_lock);
...
static kal_uint32 slim_video(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
                      MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	if(s5k3l2xx.sensorMode != IMGSENSOR_MODE_SLIM_VIDEO)
		write_setting(slim_video_setting_register_table);
	
	spin_lock(&s5k3l2xxmipiraw_drv_lock);
	s5k3l2xx.sensorMode = IMGSENSOR_MODE_SLIM_VIDEO; 
	spin_unlock(&s5k3l2xxmipiraw_drv_lock);
...
UINT32 S5K3L2XXControl(MSDK_SCENARIO_ID_ENUM ScenarioId, MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *pImageWindow, MSDK_SENSOR_CONFIG_STRUCT *pSensorConfigData)
{
    ...
    switch (ScenarioId)
    {
		...
		case MSDK_SCENARIO_ID_CAMERA_ZSD:
            ...
            break;
		case MSDK_SCENARIO_ID_HIGH_SPEED_VIDEO:
            hs_video(pImageWindow, pSensorConfigData);  // VideoHDSetting_120fps
            break;
        case MSDK_SCENARIO_ID_SLIM_VIDEO:
            slim_video(pImageWindow, pSensorConfigData); // VideoHDSetting
            break;
        default:
...
UINT32 S5K3L2XXSetVideoMode(UINT16 u2FrameRate)
{
    ...
    if(s5k3l2xx.sensorMode == SENSOR_MODE_VIDEO)
    {
		...
		S5K3L2XX_FRAMELENGTH_LOCK = MIN_Frame_length;
		spin_unlock(&s5k3l2xxmipiraw_drv_lock);
...
UINT32 S5K3L2XXMIPISetMaxFramerateByScenario(MSDK_SCENARIO_ID_ENUM scenarioId, MUINT32 frameRate) 
{
	...
	S5K3L2XXDB("[S5K3L2XX] [S5K3L2XXMIPISetMaxFramerateByScenario] scenarioId = %d , frameRate = %d\n",scenarioId, frameRate);
	...
	switch (scenarioId) {
		case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
			...
			lineLength = s5k3l2xx_preview_line_length;
			frameHeight = s5k3l2xx_preview_pixelclock / frameRate*10 / lineLength;
			if(frameHeight < s5k3l2xx_preview_frame_length)
				frameHeight = s5k3l2xx_preview_frame_length;
			dummyLine = frameHeight - s5k3l2xx_preview_frame_length;

			if(S5K3L2XX_DUMMY_LOCK == 0)
				S5K3L2XX_SetDummy(0, dummyLine);

			spin_lock(&s5k3l2xxmipiraw_drv_lock);
			s5k3l2xx.sensorMode = SENSOR_MODE_PREVIEW;
			S5K3L2XX_FRAMELENGTH_LOCK  = frameHeight;
			S5K3L2XX_DUMMY_LOCK = 0;
			spin_unlock(&s5k3l2xxmipiraw_drv_lock);
		...
		case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
			lineLength = s5k3l2xx_video_line_length;
			frameHeight = s5k3l2xx_video_pixelclock / frameRate*10 / lineLength;
			if(frameHeight < s5k3l2xx_video_frame_length)
				frameHeight = s5k3l2xx_video_frame_length;
		...
		case MSDK_SCENARIO_ID_CAMERA_ZSD:	
			lineLength = s5k3l2xx_capture_line_length;
			frameHeight =  s5k3l2xx_capture_pixelclock / frameRate*10 / lineLength;
			if(frameHeight < s5k3l2xx_capture_frame_length)
				frameHeight = s5k3l2xx_capture_frame_length;
...
UINT32 S5K3L2XXMIPIGetDefaultFramerateByScenario(MSDK_SCENARIO_ID_ENUM scenarioId, MUINT32 *pframeRate) 
{
	switch (scenarioId) 
	{
        ...
        case MSDK_SCENARIO_ID_CAMERA_3D_CAPTURE:    
			...
			break;		
		case MSDK_SCENARIO_ID_HIGH_SPEED_VIDEO:
			*pframeRate = s5k3l2xx_hs_video_max_framerate;
			break;
		case MSDK_SCENARIO_ID_SLIM_VIDEO: 
			*pframeRate = s5k3l2xx_slim_video_max_framerate;
			break;
		default:
			break;
	}
...
UINT32 S5K3L2XXFeatureControl(MSDK_SENSOR_FEATURE_ENUM FeatureId, UINT8 *pFeaturePara, UINT32 *pFeatureParaLen)
{
    ...
    MSDK_SENSOR_ENG_INFO_STRUCT	*pSensorEngInfo=(MSDK_SENSOR_ENG_INFO_STRUCT *) pFeaturePara;
		...
		//yl add
		UINT32 *feature_data_32=(UINT32 *) pFeaturePara;
    ...
    switch (FeatureId)
    {
		...
		//yl add start	SENSOR_FEATURE_GET_CROP_INFO
		case SENSOR_FEATURE_GET_CROP_INFO:
			//LOG_INF("SENSOR_FEATURE_GET_CROP_INFO scenarioId:%d\n", *feature_data_32);
			wininfo = (SENSOR_WINSIZE_INFO_STRUCT *)(*(feature_data_32+1));
		S5K3L2XXDB("[S5K3L2XX] tsf table\n");
