...
static struct PS_CALI_DATA_STRUCT sim_ps_cali={0,0,0};
...
static int epl2182_get_ps_value(struct epl2182_priv *obj, u32 ps)
{
#ifdef CUSTOM_KERNEL_SENSORHUB
    return ps;
#else
	...
	//ps_raw_data=ps;
	if(sim_ps_cali.valid==1)
	{
		if(ps >= sim_ps_cali.close)
		{
			val = 0;  /*close*/
		}
		else if(ps <= sim_ps_cali.far_away)
		{
			val = 1;  /*far away*/
		}
		else
...
/********************************************************************/
int epl2182_write_ps_thd(struct i2c_client *client, u8 thd_low, u8 thd_high)
{
	...
	if(sim_ps_cali.valid==1)
    {
	     buf[0]=sim_ps_cali.far_away;
         buf[1]=sim_ps_cali.close;
    }
    else
	{
	...
	res = set_psensor_intr_threshold(buf[0],buf[1]);
	if(res < 0)
	{
...
static epl2182_WriteCalibration(struct PS_CALI_DATA_STRUCT *data_cali)
{
		   
	  if(data_cali->valid == 1)
	  {  
		sim_ps_cali.close = data_cali->close;
		sim_ps_cali.far_away= data_cali->far_away;
		sim_ps_cali.valid = 1;
	  }	  
}
/*----------------------------------------------------------------------------*/
    ...
    switch (cmd)
    {
        ...
        case ALSPS_GET_PS_DATA:
            ...
            //dat = gRawData.ps_state;
             dat = epl2182_get_ps_value(obj, obj->ps);
        ...
        case ALSPS_GET_ALS_RAW_DATA:
			...
			printk("IOctl()----ALSPS_GET_ALS_RAW_DATA_TO_LUX\n");
			
			if((err = epl2182_read_als(obj->client, &obj->als)))
			{
				goto err_out;
			}
			//dat = obj->als;
			dat = (obj->als*obj->lux_per_count)/1000;
			if(copy_to_user(ptr, &dat, sizeof(dat)))
			{
				err = -EFAULT;
				goto err_out;
		...
		case ALSPS_GET_PS_THD:    //ps_threshold
	                if(sim_ps_cali.valid==1)
			{
				ps_cali_temp.close=sim_ps_cali.close;
				ps_cali_temp.far_away=sim_ps_cali.far_away;
				ps_cali_temp.valid=sim_ps_cali.valid;
			}
			else
			{
			...
			epl2182_WriteCalibration(&ps_cali_temp);
                        printk("[ALS/PS] ALSPS_SET_PS_CALI %d,%d,%d\n\r",ps_cali_temp.close,ps_cali_temp.far_away,ps_cali_temp.valid);
			epl2182_write_ps_thd(obj->client, atomic_read(&obj->ps_thd_val_low),atomic_read(&obj->ps_thd_val_high));
			break;
                /*--------------------------------------mly add  end-----------------------------------------*/
...
static int ps_get_data(int* value, int* status)
{
    ...
    //*value = gRawData.ps_state;
    *value = epl2182_get_ps_value(epl2182_obj->client, &epl2182_obj->ps);
    *status = SENSOR_STATUS_ACCURACY_MEDIUM;
#endif //#ifdef CUSTOM_KERNEL_SENSORHUB
