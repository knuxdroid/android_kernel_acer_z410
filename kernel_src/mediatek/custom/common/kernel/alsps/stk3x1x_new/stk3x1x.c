...
*/
#define new_arch   //pei_add
#ifdef STK_TUNE0
	#define STK_MAX_MIN_DIFF	130 //200 
	#define STK_LT_N_CT	70		//100
	#define STK_HT_N_CT	120		//150
...
static struct sensor_init_info stk3x1x_init_info = {
		...
		.uninit = alsps_remove,  //stk3x1x_local_uninit,
...
static int stk_ps_tune_zero_func_fae(struct stk3x1x_priv *obj)
{
	...
	if(gloable_vaild ==0) //pei_add
	{
		...
		APS_ERR(" pxs_psensor_debug_xx	 high: %d,low:%d ,%s \n", obj->ps_high_thd_val,obj->ps_low_thd_val ,__func__);
...
int stk3x1x_setup_eint(struct i2c_client *client)
{
  ...
  APS_LOG("ALS/PS interrupt pin = %d\n", GPIO_ALS_EINT_PIN);		
	...
	mt_eint_registration(CUST_EINT_ALS_NUM, CUST_EINT_ALS_TYPE, stk3x1x_eint_func, 0);//modify
	mt_eint_unmask(CUST_EINT_ALS_NUM);
#endif  	
    return 0;
}
...
//pei_add for aal	
		...
		acer_ALS_data =value; 	/*Acer 20141031 add*/
		return value;
#endif
...
#endif
{
	...
	switch (cmd)
	{
		case ALSPS_SET_PS_MODE:
					APS_ERR("pxs_psensor_debug ALSPS_SET_PS_MODE \n");	
		...
		case ALSPS_GET_PS_MODE:
					APS_ERR("pxs_psensor_debug ALSPS_GET_PS_MODE \n");	
		...
		case ALSPS_GET_PS_DATA:   
			APS_ERR("pxs_psensor_debug ALSPS_GET_PS_DATA \n");
			if((err = stk3x1x_read_ps(obj->client, &obj->ps)))
			{
				goto err_out;
			}
		...
		case ALSPS_GET_PS_RAW_DATA:   
			//	APS_ERR("pxs_psensor_debug ALSPS_GET_PS_RAW_DATA \n");	
			if((err = stk3x1x_read_ps(obj->client, &obj->ps)))
			{
				goto err_out;
			}
		...
		case ALSPS_SET_ALS_MODE:
						APS_ERR("pxs_psensor_debug ALSPS_SET_ALS_MODE \n");		
			if(copy_from_user(&enable, ptr, sizeof(enable)))
			{
				err = -EFAULT;
				goto err_out;
		...
		case ALSPS_GET_ALS_MODE:
				APS_ERR("pxs_psensor_debug ALSPS_GET_ALS_MODE \n");		
		...
		case ALSPS_GET_ALS_DATA: 
					APS_ERR("pxs_psensor_debug ALSPS_GET_ALS_DATA \n");		
			if((err = stk3x1x_read_als(obj->client, &obj->als)))
			{
				goto err_out;
			}
		...
		case ALSPS_GET_ALS_RAW_DATA: 
			APS_ERR("pxs_psensor_debug ALSPS_GET_ALS_RAW_DATA \n");		
			if((err = stk3x1x_read_als(obj->client, &obj->als)))
			{
				goto err_out;
			}
...
*/		
	...
	APS_ERR("pxs_psensor_debug ALSPS_GET_PS_TEST_RESULT \n");				
			if((err = stk3x1x_read_ps(obj->client, &obj->ps)))
			{
				goto err_out;
			}
		...
		APS_ERR("pxs_psensor_debug ALSPS_IOCTL_CLR_CALI \n");			
		...
		case ALSPS_IOCTL_GET_CALI: 
			APS_ERR("pxs_psensor_debug ALSPS_IOCTL_GET_CALI \n");
		...
		case ALSPS_IOCTL_SET_CALI:  
			APS_ERR("pxs_psensor_debug ALSPS_IOCTL_SET_CALI \n");			
...
/*			
			...
			if((err = stk3x1x_write_ps_low_thd(obj->client, atomic_read(&obj->ps_low_thd_val))))
				goto err_out;    
			APS_LOG("pxs_psensor_debug ALSPS_IOCTL_SET_CALI : ps_cali_result = %d\n",ps_cali_result); 
			APS_LOG("pxs_psensor_debug ALSPS_IOCTL_SET_CALI : obj->ps_cali:%d high:%d low:%d\n", obj->ps_cali, 
													atomic_read(&obj->ps_high_thd_val), 
													atomic_read(&obj->ps_low_thd_val)); 
*/													
			break;					
...
{	
	...
	enable = test_bit(STK_BIT_ALS, &stk3x1x_obj->enable) ? (1) : (0);
	if(enable && (acer_ALS_data !=0) )
		enable = acer_ALS_data;
exit: 
	return enable;
}
EXPORT_SYMBOL_GPL(acer_get_als_mode);
/*----------------------------------------------------------------------------*/
...
static int stk3x1x_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	obj->als_correct_factor = 300 ;//pei_modify
#else
	obj->als_correct_factor = 1000;
#endif
	atomic_set(&obj->ps_high_thd_val, obj->hw->ps_high_thd_val);
	atomic_set(&obj->ps_low_thd_val, obj->hw->ps_low_thd_val);	
	APS_ERR("als/ps pxs_psensor_debug_xx high:%d , %s  \n", obj->ps_high_thd_val , __func__ );	
	APS_ERR("als/ps pxs_psensor_debug_xx low:%d ,  %s  \n", obj->ps_low_thd_val , __func__ );	
