...
static unsigned long g_release_macro=0; //pei_add
...
u8 gloable_eeprom_data;
u8 gloable_eeprom_data_complement;

u8 HallOff;	  		//Hall Offset
u8 HallBiase;			//Hall Biase 

u16 Hall_Max;     		//Infinity position
u16 Hall_Min; 	   		//Macro position
...
//pei_add for liteon tdk
	if( 0x02 ==gloable_eeprom_data ) //liteon 
	{
		position = ~(position)+1; 
			
		LC898212AFDB("[LC898212AF] liteon go in\n");
...
static void readCalibrationData()
{
	...
	for(i=0;i<=0x0C;) // 0x0A
	{
		...
		LC898212AFDB("[LC898212AF] readCalibrationData:Reg[%x] =%x, Reg[%x]'s complement=0x%x\n",(i-1),eeprom_reg_read_data1,(i-1),eeprom_reg_read_data1_complement);
	...
	s4LC898212AF_Reg_Write(0x29,HallBiase);
	LC898212AFDB("[LC898212AF] readCalibrationData:HallBiase =0x%x\n",HallBiase);


	HallOff=s4AF_EEPROM_Reg_Read(0x01);
	s4LC898212AF_Reg_Write(0x28,HallOff);
	LC898212AFDB("[LC898212AF] readCalibrationData:HallOff =0x%x\n",HallOff);
	...
	eeprom_reg_read_data1=s4AF_EEPROM_Reg_Read(0x02);
	eeprom_reg_read_data2=s4AF_EEPROM_Reg_Read(0x03);
	if (gloable_eeprom_data ==0x02 )//liteon tdks ,g_release_inf is 16bit rather than 10bit 
		{
			g_release_inf= (eeprom_reg_read_data1<<8) | eeprom_reg_read_data2;	
			g_release_inf = AF_convert_2(g_release_inf);
		}
	else  //ability is 10bit ,no need to convert
		{
	...
	LC898212AFDB("[LC898212AF] readCalibrationData:g_release_inf =0x%x, %d\n",g_release_inf,g_release_inf);

//pei_add 0x04 0x05
	eeprom_reg_read_data1=s4AF_EEPROM_Reg_Read(0x04);
	eeprom_reg_read_data2=s4AF_EEPROM_Reg_Read(0x05);
	if (gloable_eeprom_data ==0x02 )//
		{
		...
		{
			g_release_macro=((eeprom_reg_read_data2<<8) | eeprom_reg_read_data1);
		}
	
	LC898212AFDB("[LC898212AF] readCalibrationData:g_release_macro =0x%x, %d\n",g_release_macro,g_release_macro);



	eeprom_reg_read_data1=s4AF_EEPROM_Reg_Read(0x08/*0x06*//*0x02*/);
	eeprom_reg_read_data2=s4AF_EEPROM_Reg_Read(0x09/*0x07*//*0x03*/);
	if (gloable_eeprom_data ==0x02 )//liteon tdks 
		{
	...
	LC898212AFDB("[LC898212AF] readCalibrationData:Hall_Max_Pos =0x%x\n",Hall_Max_Pos);
	...
	LC898212AFDB("[LC898212AF] readCalibrationData:Hall_Min_Pos =0x%x\n",Hall_Min_Pos);
...
static int LC898212AF_Open(struct inode * a_pstInode, struct file * a_pstFile)
{
    ...
    int i=0;
	//pei_add
	gloable_eeprom_data=s4AF_EEPROM_Reg_Read(0x0C); // abilitycorp 0x00 or 0x01 or 0x55 ; lite on 0x02
	gloable_eeprom_data_complement=((u8)~gloable_eeprom_data)+1;
	LC898212AFDB("[LC898212AF][peixuesongaf] readCalibrationData:Reg[0X0C]=0x%x,	Reg[0X0C]'s complement=0x%x\n",gloable_eeprom_data,gloable_eeprom_data_complement);
...
// 0x02	
	if (gloable_eeprom_data ==0x02 )//liteon  
	{
		LC898212AFDB("[LC898212AF][peixuesongaf] readCalibrationData:Reg[0X0C]=0x%x,	Reg[0X0C]'s complement=0x%x\n",gloable_eeprom_data,gloable_eeprom_data_complement);
		...
		s4LC898212AF_RAM_Write(0x02,0x0000); 
		s4LC898212AF_RAM_Write(0x18,0x0000);//Grant
		s4LC898212AF_Reg_Write(0x88,0x70);		//Poi_TDK
	
		s4LC898212AF_RAM_Write(0x28,0x8080);
		s4LC898212AF_RAM_Write(0x4C,0x4000);	//Poi_TDK
	
		s4LC898212AF_Reg_Write(0x83,0x2C);		//Poi_TDK
		s4LC898212AF_Reg_Write(0x85,0xC0);
		...
		s4LC898212AF_Reg_Write(0x99,0x00);
		s4LC898212AF_Reg_Write(0x9A,0x00);
		s4LC898212AF_Reg_Write(0x92,0x10);
		//CsHalReg
		s4LC898212AF_Reg_Write(0x76,0x0C);
		s4LC898212AF_Reg_Write(0x77,0x50);
		s4LC898212AF_Reg_Write(0x78,0x40);
		s4LC898212AF_Reg_Write(0x79,0x00);		//Poi_TDK
		s4LC898212AF_Reg_Write(0x86,0x60);		//Poi_TDK
		s4LC898212AF_Reg_Write(0xF0,0x00);
		s4LC898212AF_Reg_Write(0xF1,0x00);
	
		//CsHalFil
		s4LC898212AF_RAM_Write(0x30,0x0000);
		s4LC898212AF_RAM_Write(0x40,0x8010);	//Poi_TDK
		s4LC898212AF_RAM_Write(0x42,0x7150);	//Poi_TDK
		s4LC898212AF_RAM_Write(0x44,0x8F90);	//Poi_TDK
		s4LC898212AF_RAM_Write(0x46,0x61B0);	//Poi_TDK
		s4LC898212AF_RAM_Write(0x48,0x65B0);	//Poi_TDK
		s4LC898212AF_RAM_Write(0x4A,0x2870);	//Poi_TDK
		s4LC898212AF_RAM_Write(0x4C,0x4030);	//Poi_TDK
		s4LC898212AF_RAM_Write(0x4E,0x7FF0);	//Poi_TDK
		s4LC898212AF_RAM_Write(0x50,0x04F0);
		s4LC898212AF_RAM_Write(0x52,0x7610);
		s4LC898212AF_RAM_Write(0x54,0x16C0);	//Poi_TDK
		s4LC898212AF_RAM_Write(0x56,0x0000);
		s4LC898212AF_RAM_Write(0x58,0x7FF0);
		s4LC898212AF_RAM_Write(0x5A,0x0680);
		s4LC898212AF_RAM_Write(0x5C,0x72F0);
		...
		s4LC898212AF_RAM_Write(0x6E,0x0000);
		s4LC898212AF_RAM_Write(0x70,0x0000);
		s4LC898212AF_RAM_Write(0x72,0x18E0);	//Poi_TDK
		s4LC898212AF_RAM_Write(0x74,0x4E30);	//Poi_TDK
	
		s4LC898212AF_Reg_Write(0x86,0x60);
	}	

// 0x00 , 0x01 ,0x55  abilitycorp
else
{
	LC898212AFDB("[LC898212AF][peixuesongaf] readCalibrationData:Reg[0X0C]=0x%x,	Reg[0X0C]'s complement=0x%x\n",gloable_eeprom_data,gloable_eeprom_data_complement);
...
if (gloable_eeprom_data ==0x02 )//liteon
{
	if(AF_convert_2(ram_read_data) > g_release_inf && AF_convert_2(ram_read_data) > 900) {
		while(1) {
			i++;
			CurPos = AF_convert_2(ram_read_data);
			if(CurPos < 900 || i > 16) { //900 or 400
				mdelay(30);
                		break;
			} else {
				s4LC898212AF_WriteReg(CurPos - 70);    //70 or 140
				ram_read_data=s4LC898212AF_RAM_Read(0x3C);
			}
		}
	} else if(AF_convert_2(ram_read_data) < g_release_inf && AF_convert_2(ram_read_data) < 100) {
...
static int af_info_read_proc(char *page, char **start, off_t off, int count, int *eof, void *data)
{
    ...
    printk("%s\n", __func__);
    if(af_sorting_val==2)
        ptr += sprintf( ptr, "AF Test PASS!\nAF_read_val=0x%x;count_loop=%d;stage=%d\n",af_read_val,count_loop,stage);
    else if(af_sorting_val==1)
...
static int __init LC898212AF_i2C_init(void)
{
    ...
    if(i2c_add_driver(&LC898212AF_i2c_driver)){
