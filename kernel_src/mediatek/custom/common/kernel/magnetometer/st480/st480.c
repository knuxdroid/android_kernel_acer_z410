...
#if 1//def MTK_AUTO_DETECT_MAGNETOMETER
...
static int st480_m_enable(int en)
{
	...
	if(en == 1)
	{
		...
		/* } */
		st480sensordata.controldata[7] |= SENSOR_MAGNETIC;
		atomic_set(&m_flag, 1);
		atomic_set(&open_flag, 1);
	}
	else
...
static int st480_m_set_delay(u64 ns)
{
    ...
    MSE_LOG("st480_m_set_delay = %d\n", ns);//debug

	if(value <= 20)
	{
 		value = 20;
	}
	st480sensordata.controldata[0] = value;  // Loop Delay
...
static int st480_m_get_data(int* x ,int* y,int* z, int* status)
{
	...
	*z = st480sensordata.nmz;
	status_temp = SENSOR_STATUS_ACCURACY_HIGH;
	read_unlock(&st480sensordata.datalock); 
...
static int st480_o_enable(int en)
{
	...
	if(en == 1)
	{
		...
		/* 	schedule_delayed_work(&ist_get_raw_data_work, msecs_to_jiffies(50)); */
		/* } */
		st480sensordata.controldata[7] |= SENSOR_ORIENTATION;
		atomic_set(&o_flag, 1);
		atomic_set(&open_flag, 1);
	}
	else
...
{
    int mdelay = (int)ns/1000/1000;
    st480_m_set_delay(mdelay);
    return 0;
}
...
static int st480_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	mag_data.div_o = 1000;//CONVERT_O_DIV;
