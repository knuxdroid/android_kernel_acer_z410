...
//punk
#if defined(ACER_JADEL)||defined(ACER_Z410)
#include "cust_gpio_usage.h"
#ifdef GPIO_FDD_BAND_SUPPORT_DETECT_1ST_PIN
#define BAND_INFO_PROC //LK@add
#endif
#endif

#ifdef GPIO_SUB_BOARD_DETECT_1ST_PIN
#define	SUB_BOARD_DETECT_SUPPORT
#endif
...
void pwrkey_int_handler(void)
{
    ...
    if(mt6325_upmu_get_pwrkey_deb()==1)                
    {
        ...
        if(g_boot_mode == KERNEL_POWER_OFF_CHARGING_BOOT && timer_pre != 0)
        {
                ...
                if(0/*timer_pos - timer_pre >= 7000*1000000*/)
#else
...
static int check_modem_pin_status(void)
{
        ...
        pin1 = mt_get_gpio_in(GPIO_FDD_BAND_SUPPORT_DETECT_1ST_PIN);


        mt_set_gpio_dir(GPIO_FDD_BAND_SUPPORT_DETECT_2ND_PIN, GPIO_DIR_IN);
        mt_set_gpio_mode(GPIO_FDD_BAND_SUPPORT_DETECT_2ND_PIN, 0);
        mt_set_gpio_pull_enable(GPIO_FDD_BAND_SUPPORT_DETECT_2ND_PIN, 1);
        mt_set_gpio_pull_select(GPIO_FDD_BAND_SUPPORT_DETECT_2ND_PIN, GPIO_PULL_UP);
        pin2 = mt_get_gpio_in(GPIO_FDD_BAND_SUPPORT_DETECT_2ND_PIN);
...
#ifdef SUB_BOARD_DETECT_SUPPORT
static int check_sub_board_pin_status(void)
{
        int ret = 0;
		int pin1;
        mt_set_gpio_dir(GPIO_SUB_BOARD_DETECT_1ST_PIN, GPIO_DIR_IN);
        mt_set_gpio_mode(GPIO_SUB_BOARD_DETECT_1ST_PIN, 0);
        mt_set_gpio_pull_enable(GPIO_SUB_BOARD_DETECT_1ST_PIN, 1);
        mt_set_gpio_pull_select(GPIO_SUB_BOARD_DETECT_1ST_PIN, GPIO_PULL_UP);
        pin1 = mt_get_gpio_in(GPIO_SUB_BOARD_DETECT_1ST_PIN);

		//ret = (pin2<<1) | pin1;
		ret =  pin1;
        printk("check_sub_board_pin_status:band_detect_pin=%d\n",ret);
        return ret;
...
static int band_info_read_proc(char *page, char **start, off_t off, int count, int *eof, void *data)
{
        ...
        printk("band_info_read_proc:modem_pin_status=%d, +++1\n",modem_pin_status);
        if(modem_pin_status == 0) 
            ptr += sprintf( ptr, "BAND INFO: F4R5, band1 and band5, gpio status 0 0\n");
        else if(modem_pin_status == 1)
            ptr += sprintf( ptr, "BAND INFO: F4R6, band2 and band8, gpio status 0 1\n");
        else if(modem_pin_status == 2)
            ptr += sprintf( ptr, "BAND INFO: US, F4R4, band2 and band5, gpio status 1 0\n");
        else if(modem_pin_status == 3)
            ptr += sprintf( ptr, "BAND INFO: EU, F4R3, band1 and band8, gpio status 1 1\n");
        else
...
/* Read */
#ifdef NEW_PROC_WAY
static int band_info_read(struct seq_file *m, void *v)
{
	int modem_pin_status;
#ifdef SUB_BOARD_DETECT_SUPPORT
	int sub_board_pin_status;
#endif
	...
	if(modem_pin_status == 0) 
		seq_printf(m, "BAND INFO: ERROR. gpio status 0 0\n");
//		seq_printf(m, "BAND INFO: TW. band3,band8 and band28, gpio status 0 0\n");
	else if(modem_pin_status == 1)
		seq_printf(m, "BAND INFO: US. gpio status 0 1\n");
//		seq_printf(m, "BAND INFO: KOREO. gpio status 0 1\n");
	else if(modem_pin_status == 2)
		seq_printf(m, "BAND INFO: TW. band3,band8 and band28, gpio status 1 0\n");
//		seq_printf(m, "BAND INFO: US. gpio status 1 0\n");
	else if(modem_pin_status == 3)
		seq_printf(m, "BAND INFO: EU. band1,band3,band7,and band20, gpio status 1 1\n");
	else
		seq_printf(m, "Unkown BAND\n");
#elif defined(ACER_JADEL)
	if(modem_pin_status == 0) 
		seq_printf(m, "BAND INFO: TW. gpio 0 0.");
	else if(modem_pin_status == 1)
		seq_printf(m, "BAND INFO: KOREO. gpio 0 1.");
	else if(modem_pin_status == 2)
		seq_printf(m, "BAND INFO: US. gpio 1 0.");
	else if(modem_pin_status == 3)
		seq_printf(m, "BAND INFO: EU. gpio 1 1.");
	else
		seq_printf(m, "Unkown BAND.");
#else
	/* todo */
...
#ifdef SUB_BOARD_DETECT_SUPPORT
	sub_board_pin_status = check_sub_board_pin_status();
	printk("band_info_read_proc:sub_board_pin_status =%d, +++1\n",sub_board_pin_status);
	if(sub_board_pin_status == 1) 
		seq_printf(m, "  SUB BOARD : For EU. \n");
	else
		seq_printf(m, "  SUB BOARD : For all sub board except EU.\n");
#endif
    return 0;
}
#endif
...
{
    return single_open(file, band_info_read, NULL);
}
static  struct file_operations band_info_proc_fops= { 
    .owner = THIS_MODULE,
    .open = band_info_open,
    .read = seq_read,
    .llseek = seq_lseek,
...
static struct kobject fp_kobj;

static ssize_t fp_show(char *buf)
{
   char fp[FINGERPRINT_LEN] = {'\xc1', '\x42', '\x04', '\xa0', '\x7f', '\x3b', '\x25', '\x9a'};
...
static struct sysfs_ops fp_sysfs_ops = {
    .show = sysinfo_show,
    .store = sysinfo_store
};
...
static struct kobj_type fp_ktype = {
    .sysfs_ops = &fp_sysfs_ops,
    .default_attrs = fp_attrs
};
#endif
...
#ifdef BAND_INFO_PROC /*LK@add*/
	//new way for proc create 
#ifdef NEW_PROC_WAY
    proc_create(BAND_INFO_PROC_FILE , 0, NULL, &band_info_proc_fops);
#else
...
//#if defined(ACER_JADEL)
#if 1
	ret = kobject_init_and_add(&fp_kobj, &fp_ktype, NULL, "sys_info");
	printk("[%s]: fp_kobj finished probe, ret=%d\n", __func__, ret);

	if (ret < 0)
		printk("[%s] fail to add kobject\n", "sys_info");
#endif

    xlog_printk(ANDROID_LOG_INFO, "Power/PMIC", "****[pmic_mt_init] Initialization : DONE , +++1 !!\n");
