...
static char STR_HELP[] =
    ...
    "       dpfd_ut1:channel\n"
    ;
/*Acer 20141029 for color engine debug*/
bool sunlight_content_color_engine_enable = false; //acer color function default on
// ---------------------------------------------------------------------------
//  Command Processor
// ---------------------------------------------------------------------------
...
static void process_dbg_opt(const char *opt)
{
    ...
    else if (0 == strncmp(opt, "dbg_log:", 8))
    {
		...
		printk("dbg_log:  enable= %d\n", enable);
        if (enable==1)
            dbg_log_level = 1;
        else if(enable==2)/*Acer 20141029 add for color engine debug*/
            sunlight_content_color_engine_enable = true;
        else if(enable==3)/*Acer 20141029 add for color engine debug*/
...
void ddp_debug_init(void)
{
	...
	/*debugfs = debugfs_create_file("dispsys",
		...
		debugfs = debugfs_create_file("dispsys",
             0664, NULL, (void *)0, &debug_fops); 
		i_gid_write(debugfs->d_inode, 1006);
	/*Acer 20141029 end*/        
