...
int AudioFtm::RecieverTest(char receiver_test)
{
    ...
    {
        mStreamManager->setLoopbackEnable(true);
    ...
    else // disable
    {
        ...
        mStreamManager->setLoopbackEnable(false);
    }
#else
    if (receiver_test) // enable
    {
...
int AudioFtm::LouderSPKOCTest(char left_channel, char right_channel)
{
    ...
    if (left_channel == false && right_channel == false) // disable
    {
        mStreamManager->setLoopbackEnable(false);
    }
#else
...
int AudioFtm::PhoneMic_Receiver_Loopback(char echoflag)
{
    ...
    if (echoflag == MIC1_ON) // enable
    {
        ...
        setMicEnable(AUDIO_MIC_MASK_MIC1, true);
        current_mic_device = AUDIO_MIC_MASK_MIC1;
    }
    else if (echoflag == MIC2_ON) // enable
    {
        ...
        setMicEnable(AUDIO_MIC_MASK_MIC2, true);
        current_mic_device = AUDIO_MIC_MASK_MIC2;
    }
    else if (echoflag == MIC3_ON) // enable
    {
        ...
        setMicEnable(AUDIO_MIC_MASK_MIC3, true);
        current_mic_device = AUDIO_MIC_MASK_MIC3;
    }
    else // disable
    {
        setMicEnable(current_mic_device, false);
        current_mic_device = AUDIO_MIC_MASK_NONE;
...
int AudioFtm::PhoneMic_SpkLR_Loopback(char echoflag)
{
    ...
    if (echoflag == MIC1_ON) // enable
    {
        mStreamManager->setLoopbackEnable(true); // TODO: another pcm_open for NXP??
    ...
    else // disable
    {
        ...
        mStreamManager->setLoopbackEnable(false); // TODO: another pcm_open for NXP??
    }
...
int AudioFtm::HeadsetMic_EarphoneLR_Loopback(char bEnable, char bHeadsetMic)
{
    ...
    if (bEnable) // enable
    {
        ...
        setMicEnable(AUDIO_MIC_MASK_HEADSET_MIC, true);
        current_mic_device = AUDIO_MIC_MASK_HEADSET_MIC;
    }
    else // disable
    {
        setMicEnable(current_mic_device, false);
