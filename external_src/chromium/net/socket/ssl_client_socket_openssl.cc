...
int MapOpenSSLError(int err, const crypto::OpenSSLErrStackTracer& tracer) {
  switch (err) {
    ...
    case SSL_ERROR_SYSCALL:
      LOG(ERROR) << "OpenSSL SYSCALL error, errno " << errno;
      return ERR_SSL_PROTOCOL_ERROR;
    case SSL_ERROR_SSL:
...
int NoOpVerifyCallback(X509_STORE_CTX*, void *) {
  LOG(ERROR) << "skipping cert verify";
  return 1;
}
...
class SSLSessionCache {
 public:
  ...
  void OnSessionAdded(const HostPortPair& host_and_port, SSL_SESSION* session) {
    ...
    LOG(ERROR) << "Adding session " << session << " => "
  ...
  bool SetSSLSession(SSL* ssl, const HostPortPair& host_and_port) {
    ...
    if (it == host_port_map_.end())
      return false;
    LOG(ERROR) << "Lookup session: " << it->second << " => "
...
bool SSLClientSocketOpenSSL::Init() {
  ...
  for (int i = 0; i < sk_SSL_CIPHER_num(ciphers); ++i) {
    ...
    if (disable) {
       ...
       LOG(ERROR) << "Found cipher to remove: '" << name << "', ID: " << id
...
int SSLClientSocketOpenSSL::ClientCertRequestCallback(SSL* ssl,
                                                      X509** x509,
                                                      EVP_PKEY** pkey) {
  LOG(ERROR) << "OpenSSL ClientCertRequestCallback called";
  DCHECK(ssl == ssl_);
  DCHECK(*x509 == NULL);
  DCHECK(*pkey == NULL);
...
void SSLClientSocketOpenSSL::GetSSLInfo(SSLInfo* ssl_info) {
  ...
  LOG(ERROR) << "Encoded connection status: cipher suite = "
...
int SSLClientSocketOpenSSL::DoHandshake() {
  ...
  LOG(ERROR) << "SSLClientSocketOpenSSL::DoHandshake" ;
  if (client_auth_cert_needed_) {
    ...
    if (trying_cached_session_ && logging::DEBUG_MODE) {
      LOG(ERROR) << "Result of session reuse for " << host_and_port_.ToString()
...
int SSLClientSocketOpenSSL::SelectNextProtoCallback(unsigned char** out,
  ...
  LOG(ERROR) << "next protocol: '" << npn_proto_ << "' status: " << npn_status_;
#endif
  return SSL_TLSEXT_ERR_OK;
}
...
int SSLClientSocketOpenSSL::DoVerifyCertComplete(int result) {
  ...
  if (result == OK) {
    ...
    LOG(ERROR) << "DoVerifyCertComplete error " << ErrorToString(result)
             << " (" << result << ")";
  }
...
void SSLClientSocketOpenSSL::TransportWriteComplete(int result) {
  ...
  if (result < 0) {
    ...
    LOG(ERROR) << "TransportWriteComplete error " << result;
...
void SSLClientSocketOpenSSL::TransportReadComplete(int result) {
  DCHECK(ERR_IO_PENDING != result);
  if (result <= 0) {
    LOG(ERROR) << "TransportReadComplete result " << result;
